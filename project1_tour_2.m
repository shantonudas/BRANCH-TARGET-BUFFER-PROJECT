...
for i = 1:length(PC)-1
    ...
    if PC(i) == BTB(entry+1,2) % PC alreaady in BTB Step 1, Yes condition
        ...
        if BTB(entry+1,3) == (PC(i+1)) %PC_next matches in BTB Step 2B
            ...
            BTB(entry+1,5+histry) = sm_global_2(prev_pred_global, actual_pred);
        ...
        if (PC(i+1)-PC(i)) ~= 4 % branch exists step 2A yes
            ...
            BTB(entry+1,5) = 2; % 2 = weakly taken, global 00
            BTB(entry+1,6) = 2; % 2 = weakly taken, global 01
            BTB(entry+1,7) = 2; % 2 = weakly taken, global 10
            BTB(entry+1,8) = 2; % 2 = weakly taken, global 11
